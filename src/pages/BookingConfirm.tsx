  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6">
      ...
      <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        className="text-xl font-bold font-display text-foreground text-center">Бронирование подтверждено!</motion.h1>
      ...
      <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        whileTap={{ scale: 0.98 }} onClick={() => navigate('/')}
        className="mt-8 w-full max-w-sm py-3.5 bg-primary text-accent-foreground rounded-lg font-semibold text-sm glow-green">На главную</motion.button>
    </div>
  );
};
